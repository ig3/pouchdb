...
function replicate(repId, src, target, opts, promise) {
  ...
  var current_batch = null;       // the batch currently being processed
  ...
  function writeDocs() {
    if (current_batch.docs.length === 0) {
  ...
  function onGet(err, docs) {
    ...
    Object.keys(docs).forEach(function (revpos) {
      ...
      if (doc) {
        result.docs_read++;
        current_batch.pendingRevs++;
        current_batch.docs.push(doc);
      }
    });
  ...
  function finishBatch() {
    writeCheckpoint(src, target, repId, current_batch.seq, function (err, res) {
      if (err) {
        return abortReplication('writeCheckpoint completed with error', err);
      }
      last_seq = current_batch.seq;
      result.last_seq = current_batch.seq;
  ...
  function onRevsDiff(err, diffs) {
    ...
    current_batch.diffs = diffs;
    current_batch.pendingRevs = 0;
    fetchRev();
  }
  ...
  function fetchRevsDiff() {
    var diff = {};
    current_batch.changes.forEach(function (change) {
  ...
  function processPendingBatch() {
    if (pendingBatch.changes.length === 0) {
      if (changesCompleted && batches.length === 0 && current_batch === null) {
        replicationComplete();
      }
      return;
    }
  ...
  function complete(err) {
    ...
    if (err) {
      result.status = 'src.chaanges completed with error';
      result.errors.push({
        status: 500,
        error: 'Replication aborted',
